        ...
        return (
            <div className={`bg-white transiton-all duration-300 ${font.className} size-full rounded-[18px] h-full p-[24px] group`}>
                <div className="flex flex-row justify-between h-full">
                    ...
                    <div className="lg:text-[24px] sm:text-[20px] xl:[600px] md:w-[375px] sm:w-[300px] w-[150px] text-[16px]">
                        {item.description}
                    </div>
                </div>
                <div className="flex justify-center items-center w-full">
                    <Link href = {item.link} rel="noopener noreferrer" target="_blank">
                    <button className="rounded-[50%] border-4 border-black sm:p-[32px] p-[16px] group-hover:bg-black transtion-all duration-300"
                    onMouseEnter={()=>{
                        setArrowClassName("translate-x-1 -translate-y-1")
                    }}
                    onMouseLeave={()=>{
                        setArrowClassName("")
                    }}
